"""MAMSE-013 resource benchmark: wall/CPU, RSS, swap, dense intermediate.

Usage: python scripts/mamse013_benchmark.py <wav> <out.json>
...
def measure(samples: np.ndarray, sr: int, label: str) -> dict:
    ...
    build_er_b_sketch(obs, DEFAULT_CONFIG)
    ...
    return {
        ...
        "n_channels": int(obs.channel_energies.shape[0]),
        "n_frames": int(obs.channel_energies.shape[1]),
        "status": obs.status,
