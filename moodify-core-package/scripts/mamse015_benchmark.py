"""MAMSE-015 resource benchmark: wall/CPU, RSS, swap, dense intermediate.

Usage: python scripts/mamse015_benchmark.py <wav> <out.json>
...
def measure(samples: np.ndarray, sr: int, label: str) -> dict:
    ...
    build_soft_object_sketch(obs, DEFAULT_CONFIG)
    ...
    return {
        ...
        "n_frames": int(obs.frame_probabilities.shape[0]),
        "status": obs.status,
