...
while [ "$(date +%s)" -lt "$END" ]; do
  ...
  h_web=$(curl -s -o /dev/null -w "%{http_code}" --max-time 8 "$BASE/healthz" 2>/dev/null || echo 000)
  h_ear=$(curl -s -o /dev/null -w "%{http_code}" --max-time 8 "$BASE/api/v1/health" 2>/dev/null || echo 000)
  h_music=$(curl -s -o /dev/null -w "%{http_code}" --max-time 8 "$MUSIC_BASE/api/v1/music/bootstrap" 2>/dev/null || echo 000)
  h_range=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -H "Range: bytes=0-1023" \
    "$MUSIC_BASE/audio/cadeau10-album1/je-ne-veux-pas-enfermer-ton-aujourdhui.wav" 2>/dev/null || echo 000)
