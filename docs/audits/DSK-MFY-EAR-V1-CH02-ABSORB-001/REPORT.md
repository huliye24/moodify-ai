...
- **阶段 0（本次）**：蒸馏文档 + 本审计报告 + 架构文档交叉引用。**已完成**。
- **阶段 1（1–2 个补丁包）**：MSE 命名体系与条件化接口、finding 时间尺度元数据、epistemic 词表一等化。测试驱动，先立项名。
- **阶段 2（MAMSE-013 起）**：ERB filterbank → 掩蔽推断 → 软听觉对象 → 音高证据；按 EXPERIMENTAL_ACCEPTED 流程（R1–R3）。
