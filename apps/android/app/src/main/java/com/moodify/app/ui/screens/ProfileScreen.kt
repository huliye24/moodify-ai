...
@Composable private fun QuickActions() { ProfileSection("快捷功能") { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceAround) { QuickItem(Icons.Outlined.ReceiptLong, "我的订单"); QuickItem(Icons.Outlined.CloudQueue, "云端空间", "3.2 GB / 10 GB"); QuickItem(Icons.Outlined.VerifiedUser, "版权证明"); QuickItem(Icons.Outlined.HeadsetMic, "帮助反馈") } } }
...
@Composable private fun QuickItem(icon: ImageVector, title: String, subtitle: String = "") { Column(horizontalAlignment = Alignment.CenterHorizontally) { Box(Modifier.size(45.dp).background(Color(0xFFF6F7FF), RoundedCornerShape(13.dp)), contentAlignment = Alignment.Center) { Icon(icon, null, tint = MoodifyBlue, modifier = Modifier.size(25.dp)) }; Text(title, color = MoodifyNavy, fontSize = 11.sp, modifier = Modifier.padding(top = 6.dp)); if (subtitle.isNotEmpty()) Text(subtitle, color = MoodifyMuted, fontSize = 8.sp) } }
