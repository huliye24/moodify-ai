...
@Composable private fun FeaturedTrack(song: CatalogSong?, onPlay: () -> Unit) {
    Box(Modifier.fillMaxWidth().height(250.dp).background(Brush.linearGradient(listOf(Color(0xFF312780), Color(0xFF7651D8), Color(0xFFDB8CE9))), RoundedCornerShape(24.dp))) {
        ...
        Column(Modifier.fillMaxSize().padding(18.dp)) {
            Surface(color = Color(0x55351C92), shape = RoundedCornerShape(14.dp)) { Text(if (song != null) "今日推荐" else "今日推荐", color = Color.White, fontSize = 12.sp, modifier = Modifier.padding(horizontal = 11.dp, vertical = 6.dp)) }
            ...
            Text(if (song != null) "平台母带 · ${song.preset}" else "梦境般的旋律，带你遨游星海", color = Color.White.copy(.8f), fontSize = 13.sp, modifier = Modifier.padding(top = 5.dp))
