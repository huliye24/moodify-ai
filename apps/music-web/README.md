...
Moodify 的音乐聆听平台。产品界面以声音为先，作品默认使用统一的
Moodify 黑胶视觉，创作者无需先制作封面。

线上地址：<https://rongjinwenchuan.xyz/>

## Audio assets

真实音频是部署资产，不进入普通 Git 历史。已发布曲目的文件大小与
SHA-256 记录在 `assets/cadeau10-album1.json`，用于发布与回滚验证。

播放器默认从 `https://rongjinwenchuan.xyz/audio` 读取音频；开发或部署到
其他媒体源时，设置 `NEXT_PUBLIC_AUDIO_BASE_URL`。媒体源下应保持如下结构：

```text
cadeau10-album1/<manifest 中的 file>
```

本地 `public/audio/` 仍受仓库根 `.gitignore` 保护，禁止把私人音频或大型
生成资产提交到 GitHub。
